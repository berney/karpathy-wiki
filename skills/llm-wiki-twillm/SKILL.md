...
project-root/
  ...
  docker-compose.yml     ← Optional Docker setup (may already exist or be created)
  Dockerfile             ← Optional image build file
```
...
Use a published image when available (`image: ghcr.io/ORG/twillm:latest`). If no published image exists yet, generate a Dockerfile alongside the compose file using bundled fixtures. Once a published image is available, switch from `build:` to `image:`.
