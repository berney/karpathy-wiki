...
- Timestamps must be generated by running `scripts/timestamp.py` (not hardcoded or guessed).
...
2. **Create/update tiddlers.** Write focused tiddlers for each distinct concept, entity, or finding.
  Keep them small and atomic — one concept per tiddler.
  Use `scripts/timestamp.py` to generate `created`/`modified` timestamps — never hardcode them.
  Before creating a new tiddler, call `GET /bdawg/canonical` (see `references/checking-tiddlers.md`) to check existence, shadow status, and get the canonical filename in one call.
  Link generously to existing tiddlers using wikilinks.
